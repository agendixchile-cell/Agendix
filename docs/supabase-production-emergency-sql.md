# SQL de emergencia Supabase Production

Proyecto Agendix confirmado por usuario:

- URL: `https://sbebrhlcxwmzixpzvhuq.supabase.co`
- Ref: `sbebrhlcxwmzixpzvhuq`

Validacion anon ejecutada desde `.env.local` contra REST API production:

- `public_booking_professionals`: `404 PGRST205`, no existe en schema cache/API.
- `profiles`: `206`, `anon` puede leer filas.
- `miembros_centro`: `206`, `anon` puede leer filas.
- `salas`: `206`, `anon` puede leer filas.

Estado: exposicion critica. Aplicar este SQL en Supabase SQL Editor con un usuario con permisos de owner/admin de base, idealmente despues de confirmar backup. Si no hay backup confirmado, priorizar al menos esta correccion de grants porque reduce exposicion publica directa.

## 1. Parche transaccional

Archivo ejecutable equivalente:

```bash
supabase db query \
  --db-url "postgresql://postgres:${SUPABASE_DB_PASSWORD}@db.sbebrhlcxwmzixpzvhuq.supabase.co:5432/postgres" \
  --file supabase/emergency/production-anon-rls-hotfix.sql
```
...
- Segunda query: `anon` y `authenticated` con `SELECT`.

## 3. Verificacion REST anon

Despues de aplicar el SQL, volver a ejecutar desde el repo:

```bash
/Users/estebantorrescea/.cache/codex-runtimes/codex-primary-runtime/dependencies/node/bin/node scripts/verify-supabase-anon-rls.mjs
```

Resultado esperado:

- `public_booking_professionals`: `200` o `206`.
- `profiles`, `miembros_centro`, `salas` y tablas sensibles: `401`/`403` o sin grant directo.

-- Repara y activa la primera etapa de recordatorios: emails 48h antes.
-- El historial remoto puede marcar la migracion anterior como aplicada aunque falten objetos.
...
insert into public.configuracion_recordatorios (centro_id, whatsapp_enabled)
select id, false
from public.centros
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'recordatorios_reserva_centro_id_fkey'
  ) then
    alter table public.recordatorios_reserva
      add constraint recordatorios_reserva_centro_id_fkey
...
drop trigger if exists configuracion_recordatorios_updated_at
  on public.configuracion_recordatorios;
...
begin
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'configuracion_recordatorios'
      and policyname = 'miembros pueden leer configuracion recordatorios'
  ) then
  ...
  if not exists (
    ...
    where schemaname = 'public'
      and tablename = 'configuracion_recordatorios'
      and policyname = 'admins pueden escribir configuracion recordatorios'
  ) then
  ...
  if not exists (
    ...
    from pg_policies
    where schemaname = 'public'
      and tablename = 'recordatorio_envios'
      and policyname = 'miembros pueden leer logs recordatorios'
  ) then
